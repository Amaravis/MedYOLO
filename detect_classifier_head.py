...
from utils3D.datasets import LoadNiftis,transpose_nifti_shape
from utils3D.general import non_max_suppression, scale_coords, zxyzxy2zxydwh
import cmb_classifier_head as cmb_head
import numpy as np
...
def run(weights,  # model.pt path(s)
    ...
    for path, img, im0s in dataset:
        ...
        cmb_head_model = cmb_head.load_model('class_runs/binary_classifier3d_best.pth','cuda')
        # Process predictions
        for _, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for row in det[:, :]:
                    print(row)
                    im0_new = np.transpose(im0, (2, 0, 1)) 
                    patch = cmb_head.extract_patch(im0_new, (int(row[0].item()),int(row[2].item()),int(row[1].item())))
                    patch = torch.from_numpy(patch).unsqueeze(0).unsqueeze(0).float().to(device) 
                    print(patch.shape)
                    with torch.no_grad():
                        cmb_logits = cmb_head_model(patch)
                        cmb_prob = torch.sigmoid(cmb_logits).item()
                        print(cmb_prob)
                        cmb_pred = 1 if cmb_prob > 0.5 else 0
                    
                    if cmb_pred == 1:
                        selected_rows.append(row)
                if len(selected_rows) > 0:
                    det = torch.stack(selected_rows)
                else:
                    det = torch.empty((0, det.shape[1]))
